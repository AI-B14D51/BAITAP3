...
void UCS::Input_UCS(string filein) {
    // create the graph
...
double UCS::get_solution(bool is_ds){
    ...
    fout << "UCS\n";
    fout << "Duong di ngan nhat tu " << Constants::start << " den " << Constants::end << " la: " << answer[0];
    double vm, rss;
    process_mem_usage(vm, rss);
    fout << "\nMemory: " << "VM: " << vm << " KB;\n" << "\tRSS: " << rss << "KB" << endl;
    clock_t end = clock();
    double time_taken = ((double)(end - start)) / CLOCKS_PER_SEC;
    fout << "Thoi gian UCS: " << time_taken * 1000 << " miliseconds";
    fout.close();
    return 0.0;
}
