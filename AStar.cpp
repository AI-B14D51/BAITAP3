...
void Input_AStar(string filein, string filein2, vector<vector<int>> &Matrix) {
...
void AStar_Alogithm(int n, int s, int t, vector<vector<int>> Matrix) {
    // khoi tao gia tri cho node[i].f
    for (int i = 0; i < n; i++) {
        nodes[i].f = 1e6;
    }
...
void Res_Astar(int s, int t, vector<vector<int>> Matrix) {
    fstream fout;
    ...
    fout << s<<endl;
    fout << "Do dai duong di : "<<cost<<endl;
    double vm, rss;
    process_mem_usage(vm, rss);
    fout << "Memory: " << "VM: " << vm << " KB;\n" << "\tRSS: " << rss << "KB" << endl;
    fout.close();
}
...
double Astar::get_solution(bool is_ds)
{
    ...
    Input_AStar(Constants::FILEIN, Constants::FILEIN2, Matrix);
    AStar_Alogithm(Constants::n_nodes, Constants::start, Constants::end, Matrix);
    Res_Astar(Constants::start, Constants::end, Matrix);
    clock_t end = clock();
    double time_taken = ((double)(end - start)) / CLOCKS_PER_SEC;
    fstream fout;
    fout.open("output.out", ios::app);
    fout << "Thoi gian Astar: " << time_taken * 1000 << " miliseconds";
    fout.close();
	return 0.0;
}
