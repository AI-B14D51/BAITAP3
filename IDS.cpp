...
void IDS::Input_IDS(string filein)
{
    fstream fin;
    int u, v, w;
...
void IDS::IDS_exe(string filein, int src, int goal, int limit)
{
    clock_t start = clock();
    fstream fout;
    ...
    fout << "\nThoi gian IDS: " << time_taken * 1000 << " miliseconds";
    double vm, rss;
...
double IDS::get_solution(bool is_ds){
    IDS_exe(Constants::FILEIN, Constants::start, Constants::end, Constants::limit);
    return 0.0;
}
